...
func (group *RouterGroup) Group(prefix string) *RouterGroup {
	...
	// CRITICAL ELABORATION: Copying the slice safely
	// We must create a fresh slice and copy the parent's middlewares into it.
	// If we just assigned `newGroup.middlewares = group.middlewares`, modifying
	// the child group's slice would accidentally mutate the parent group due to
	// Go's underlying array referencing.
