...
func (r *router) getRoute(method string, path string) (*node, map[string]string) {
	...
	if n != nil {
		...
		for index, part := range parts {
			if part[0] == ':' {
				// Map the extracted value to the parameter name (e.g., params["id"] = "123")
				params[part[1:]] = searchParts[index]
			}
			if part[0] == '*' && len(part) > 1 {
				// Reconstruct the remaining path for wildcards
...
// search recursively traverses the tree to find a matching pattern
